...
public class SftpUtil implements AutoCloseable {
     ...
     * @throws JSchException JSchException
     */
    public ApiResult<Object> connectServerUseExec(String command, String... info) throws JSchException, IOException {
        final String serverIP = info[0];
        final String username = info[2];
        this.getSession(serverIP, Integer.parseInt(info[1]), username, info[3]);
        // 打开SFTP通道
        ...
        if (exitStatus == 0) {
            ...
            successApiResult.setContent(execLog);
            return successApiResult;
        } else {
            ApiResult<Object> failureApiResult = ApiResult.initFailure();
            failureApiResult.setMessage("执行失败！");
            failureApiResult.setContent(execLog);
